...
再按功能顺序作 Z 字单页组合、统一行高、整体平移。连接数据不随分页变化；
所有 pin→net 与 NC 都必须有意图。不能从空画布推断被清掉的功能电路。
默认页边距、框内最小边距、模块间距均为 10 raw（0.1 inch/2.54 mm），标题净距为
5 raw，字高20。框内边距会因网格向外取整和统一行高增大；图签 keepout 必须避让。
...
先压缩各模块的框和标题包络，再按功能顺序从纸张左上角向右排模块，宽度不足时进入下一行。各行统一使用最大模块
高度，同行框的上下边对齐；不按原始随机 XY 排序。模块内器件、所有 pin、wire 端点
和电源标记整体平移，连接和方向不变。单页装不下即报错，不自动增加分页。
共享行规划器已用五模块、两行数据回归；当前完整落地入口仍为四器件 POWER。
