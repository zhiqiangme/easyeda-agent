...
候选版本 **v1.4.2**。v1.4.1 已用于公开预览，不能覆盖；本次统一 CLI 构建标记、
连接器 manifest/npm 元数据及 Skill 到 1.4.2。变更内容见
[Changelog](../extension/CHANGELOG.md)。本页记录准备与验收边界，不表示已经发布。
...
2026-09-06 本地准备检查通过：Go 全量测试、185 项连接器测试、18 项发布/安装态
...
及跨页合并，不算现场 Apply。入口 223→90 行，公共 Markdown 528,097→311,905 字节，
减少 40.9%；统计不含用户本地草稿。当前产物位于 `dist/`。
