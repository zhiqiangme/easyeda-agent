...
README 展示使用真实原理图的官方导图。静态图片直接来自
...
2026-09-08 在已有「宏恩门禁底座载板」工程采集：两张静图为 `POWER_RF_MCU` 与
`TALK_RF_SD` 的完整官方导图，GIF 记录 `POWER_RF_MCU` 从首件放置到功能框完成的
12 个真实阶段。动图宽 1440 像素，约 8.8 秒、1.01 MiB；使用差分帧压缩，末帧停留后循环。
