...
| S6′ | 呈现与图签 | 模块呈现数据→Apply(粉色虚线框 + 0.2 inch 标题),`sch titleblock` 填图签 | 回读几何/样式,`sch check` 无 missing-partition/titleblock;1.4 不要求 Notes |
...
- **认领 + 画框**(动作细节见 [`schematic-placement.md`](./schematic-placement.md) 的 *Functional frames + text labels*):
  ...
  5. **单页小板也要画区框** —— 「最小 / 单页」不是省略分区的借口。可以不分页,
     但区名框与标题照画。
- **模块框和标题必须进入呈现数据与转换队列**:粉色虚线框、粉色标题,字高 0.2 inch
  (20 原理图坐标单位)。先计算器件、线、符号和文字的包络,再 Apply 并回读。
  1.4 不生成独立 Notes,缺少 Notes 不阻塞交付。
...
- **机械兜底(别靠记忆)**:`sch check` 有 **`missing-partition`** 检查项——多器件页
  ...
  **判据的证人改成了画布(#181,流程不变)**:认框靠页上那条**区标题文本**
  ...
  组只影响报文措辞)。(挂账:`sch zones status` / `sch status` 仍只读记账。)
...
- **有 ERROR 先回 S3 调整**:单件 `sch modify`;成排对齐 `sch align --mode centerx/top…`;
  ...
  (整板放置每 ~10 件存一次,别等全放完)。
- **模块框、标题与图签是当前呈现验收内容**:
  - 默认先一页,每个 Lib 的核心及外围保持连续实线连接。
  - 模块框为粉色虚线,左上标题粉色、字高 0.2 inch(20单位);二者由数据计算、转换并回读。
  - `sch check` 仍检查 `missing-partition` 与 `missing-titleblock`,`sch gate --strict` 会阻塞这些缺项。
  - 1.4 移除独立 `sch note` 入口及 `missing-note` / `note-outside-zone` 门。
    `sch text-list` 保留普通文字读取,既有文字的碰撞仍纳入几何检查。
