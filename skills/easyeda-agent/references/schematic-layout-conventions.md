...
## 1.5 分页分区 + 电路说明（默认必做，不是可选装饰）

> 来源：用户反馈——agent 产出的原理图常常一页塞满、无分区框、无任何文字说明，
> 电气上对但**交付可读性差**。行业惯例是「先看区、再看线」：审图人先靠分区和
> 说明建立地图，再进电路细节。以下三件套是**布局默认**，档位速查表（SKILL.md）
> 与 design-flow S1–S3 的过门条件都引用这里。
...
2. **分区框 + 区名（S2）**——`sch block-apply` 落块时已按**功能子群**自动归组,
   ...
   **一个虚拟组 / zone 认领 = 一个分区框**(与 `sch zone-arrange` 的区一一对应,
   同一把尺;不会把相邻的两个区并成一个大框),框 = 成员虚拟组体积的并集
   (框住内容是构造保证),区名在框内左上、电路说明在框内左下
   ...
   > `partitionOverlap` 非 0 = 两个区的体积**真的**互相压 → `sch zone-arrange --apply`
   > 重排或 `sch group-move` 挪件;调 `--gutter` 治不了,也没有「合并成一个框」
   > 这条退路(那只是把重叠藏起来)。反过来:`zone-arrange` 断言③ 绿的页面,
   > `zone-draw` 一定画得出来。
3. **电路说明（S3，每模块 1~3 行）**——`easyeda sch note` 在模块框下/旁放简短
   说明文本：**作用 + 关键参数 + 设计要点**。字号默认 10（低于区名 14/分区题 22），
   灰色 `#5A5A5A`，读起来是注释不是电路。多行用 `\n`。

**合格说明的样子（写参数和决策，不写废话）：**

```
LDO: 5V→3V3 1A\n输入/输出各 100nF+10µF 去耦          ← 电源模块
BOOT: GPIO0 拉低进烧录\nEN: 10k 上拉 + 100nF 延时     ← strap/按键
CH340C: USB 全速\nDTR/RTS→Q1/Q2 自动下载             ← USB-串口
天线: 50Ω 走线, π 网络预留\n馈线下方禁铺铜             ← RF
```

反例：「这是电源部分」「MCU 模块」（区名框已经说了，note 里重复一遍=废话）。

**摆放与核对**：`--zone <区>` 时 note **贴着分区框底边**落进框内说明带
（`note.y = 框.minY + 16`；文字锚点是块的**左下角**、块向上生长，所以离框底的
距离与行数/字号无关，同页所有说明底边齐平）。其余情况 note 放模块框内/旁的
**空白处**，不压器件不压导线——放完跑
